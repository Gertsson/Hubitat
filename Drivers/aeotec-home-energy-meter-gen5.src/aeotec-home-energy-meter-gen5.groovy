 ...
 *
 *	Copyright 2023 Jonas Gertsson (all original code made by Artur Draga - ClassicGOD)
 *  Changes made are:
...
metadata {
	definition (name: "Aeotec Home Energy Meter Gen 5",namespace: "Gertsson", author: "Jonas Gertsson") {
...
def zwaveEvent(hubitat.zwave.commands.meterv3.MeterReport cmd, Integer ep=null) {
    ...
    switch (cmd.scale) {
        case 0: type = "energy";  unit = "kWh"; decimal = "false"; break; 
        case 1: type = "totalEnergy"; unit = "kVAh";  decimal = "false"; break;
        case 2: type = "power"; unit = "W"; decimal = "false"; break;	
        case 4: type = "voltage"; unit = "V";  decimal = "false"; break;    
        case 5: type = "current"; unit = "A";  decimal = "true"; break;
    }
    if (ep == null) {
        if (decimal == "false") sendEvent([name: type, value: (int)cmd.scaledMeterValue.doubleValue(), unit: unit])
   	    else sendEvent([name: type, value: cmd.scaledMeterValue.doubleValue().round(2), unit: unit])
    }    
    else {
        if (decimal == "false") getChildDevice("${device.deviceNetworkId}-${ep}")?.sendEvent([name: type, value: (int)cmd.scaledMeterValue.doubleValue(), unit: unit])
        else getChildDevice("${device.deviceNetworkId}-${ep}")?.sendEvent([name: type, value: cmd.scaledMeterValue.doubleValue().round(2), unit: unit]) 
    }     
}
